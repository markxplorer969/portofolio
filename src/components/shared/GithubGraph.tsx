  ...
  useEffect(() => {
    const fetchGitHubData = async () => {
      try {
        ...
        const response = await fetch('https://github-contributions-api.jogruber.de/v4/markxplorer969?y=last');
        ...
        <div className="w-full max-w-4xl mx-auto">
          <div className="flex items-center justify-center p-8">
            <div className="text-center">
              ...
              <h3 className="text-xl font-semibold text-white mb-2">GitHub Activity Unavailable</h3>
              <p className="text-zinc-400 text-sm max-w-md">{error}</p>
              <a 
                href="https://github.com/markxplorer969" 
                target="_blank" 
        ...
        <div className="relative bg-zinc-900/50 border border-zinc-800 rounded-3xl p-8 overflow-hidden backdrop-blur-sm">
          ...
          <div className="mb-8 flex items-center justify-between">
            <div>
              ...
              <p className="text-zinc-400 text-sm">My open source activity log over the last year.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="text-center">
                <div className="text-2xl font-bold text-indigo-400 mb-1">
                  {data.reduce((sum, week) => sum + week.contributionDays.reduce((weekSum, day) => weekSum + day.contributionCount, 0), 0)}
                </div>
                <div className="text-zinc-400 text-sm">Total Contributions</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-cyan-400 mb-1">
                  {data.filter(week => week.contributionDays.some(day => day.contributionCount > 0)).length}
                </div>
                <div className="text-zinc-400 text-sm">Active Weeks</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-purple-400 mb-1">
                  {Math.max(...data.flatMap(week => week.contributionDays.map(day => day.contributionCount)), 0)}
                </div>
